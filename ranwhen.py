...
line_pattern = re.compile(
    "reboot\s+system\s+boot\s+\w{3}\s+([\d\w\s:]{20})\s+-\s+\w{3}\s+([\d\w\s:]{20})")
...
while current_time > earliest_time:
    ...
    half_hour_index = current_time.hour * 2 + \
        (1 if current_time.minute >= 30 else 0)
...
grid_header = style_text(
    "▆           ▆           ▆           ▆           ▆", fgcolor=grid_color)
grid_footer = style_text(
    "▀           ▀           ▀           ▀           ▀", fgcolor=grid_color)
...
print(style_text("Daily average:      ", bold=True) +
      format_delta(total_time / number_of_days))
